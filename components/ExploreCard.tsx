...
const ExploreCard = ({ id, pageUrl, imgUrl, title, href, status, index, active, handleClick }:
    {
        id: string,
        imgUrl: string,
        pageUrl: string,
    ...
    return (
        <motion.div
            ...
            onClick={() => { active == id ? router.push(pageUrl) : handleClick(id) }}
        >
            <Image
                src={imgUrl}
                fill
            ...
            {active !== id ? (
                ...
                <div className="absolute bottom-0 p-8 flex justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
                    ...
                    <h2 className="rounded-md bg-gradient-to-r from-indigo-600 to-pink-500 mt-[24px] pl-2 font-semibold sm:text-[32px] text-[24px] text-white">
                        {title}
                    </h2>
                </div>
